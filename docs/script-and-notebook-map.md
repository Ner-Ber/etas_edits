...
Replace `<REMOVAL_COMMIT>` with `06e6969` ("Remove poisson-on-grid continuation stack").
...
| `runnable_code/run_continuation_models.py` | Neri Berman | `config/continuation_models_config.json` | Single config: any subset of `etas` / `thinning` / `thinning_magnet`; model-keyed cache |
| `runnable_code/continuation_compare.py` | Neri Berman | `config/catalog_california_etas_vs_thinning_config.json` | inversion → classic ETAS **and** Ogata thinning side-by-side; HTML report |
| `runnable_code/continuation_ensemble.py` | Neri Berman | same | N runs of **one** method: `etas` / `thinning` / `thinning_magnet` |
| `runnable_code/catalog_california_etas_vs_thinning_ensemble.py` | Neri Berman | same + `n_runs` | paired ETAS+thinning ensemble (legacy name) |
| `runnable_code/continuation_ensemble.py` | Neri Berman | same | N runs of **one** method: `etas` / `thinning` / `thinning_magnet` |
...
### 1b. Unified 3-model runner (preferred for catalog continuation)

```
config/continuation_models_config.json
  → runnable_code/run_continuation_models.py
      → optional MAGNET load/train (magnet.mode)
      → etas.inversion (cached under <output_root>/inversions/)
      → etas / thinning / thinning_magnet under
        <output_root>/<method>/inv_<id>/seed_<seed>/
```
...
runnable_code/continuation_compare.py   [Neri Berman]  # single run
  OR
...
|----------|--------|------------------|-------|
| `notebooks/catalog_california_etas_vs_thinning_continuation.ipynb` | Neri Berman | *(prototype)* → `continuation_compare.py` | Single-run ETAS vs thinning on real catalog |
...
| `notebooks/compare_thinning_vs_thinning_magnet_ensemble.ipynb` | *(untracked)* | `continuation_ensemble.py` | `thinning` vs `thinning_magnet` ensembles |
...
| `config/continuation_models_config.json` | `run_continuation_models.py` |
...
| Compare ETAS vs Ogata thinning on California/Hauksson | `run_continuation_models.py` or `prepare_hauksson_inputs.py` → ensemble scripts |
| Full MAGNET + ETAS | `runnable_code/MAGNET_ETAS_pipeline.py` or `run_continuation_models.py` with `thinning_magnet` |
